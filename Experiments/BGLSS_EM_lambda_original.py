def BGLSS_EM_lambda(Y, X, group_size, num_update = 100, niter = 100, a = 1, 
    ...
    matlambda2_path = np.repeat(-1.0, ngroup*num_update).reshape(num_update, ngroup)
    l = np.zeros(ngroup)
    beta = []
    ...
    for update in range(num_update):
        ...
        tau2_each_update = np.array([]).reshape(ngroup, 0)
        for itera in range(niter):
            ...
            for i in range(ngroup):
                if Z[i] == 0:
                    tau2[i] = np.random.gamma((group_size[i]+1)/2, scale = 2/matlambda2[i])
                else:
                    tau2[i] = 1/np.random.wald(math.sqrt(matlambda2[i]*sigma2/np.sum(beta[i]**2)), matlambda2[i])
            tau2_each_update = np.hstack([tau2_each_update, tau2.reshape(ngroup,1)])
            s = 0
            for i in range(ngroup):
        ...
        lambda2 = (p+ngroup)/np.sum(tau2_mean)
        matlambda2 = np.repeat(lambda2,ngroup)
